...
export async function getReportsByStatus(status: ReportStatus, rtId?: string) {
  try {
...
export function nextStep(currentStatus: ReportStatus): ReportStatus {
...
export async function updateReportStatus(
  ...
  try {
    const updatedReport = await prisma.$transaction(async (tx) => {
      const newStatus = nextStep(report.status!);
